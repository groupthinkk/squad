(function (window, $) {
	...
	function timerExpired() {
		alert('Time\'s up!');
		// TODO: What should happen when the timer expires?
	}
	...
	function showImages(left, right) {
		var loaded = 0;
		function ready() {
			if (++loaded == 2) {
				startTimer(5);
		...
		$('#left img').one('load', ready).attr('src', left);
		$('#right img').one('load', ready).attr('src', right);
	}

	function showBadge(type) {
		$('.badger.' + type).fadeIn('fast');
		$('.badger').delay(500).fadeOut('fast');
	}

	$('.image').click(function() {
		stopTimer();
		$('.image').fadeOut('fast');
		var choice = $(this).attr("id")==='right' ? 'b' : 'a';
		reportClick(currentPackageId, choice, function (response) {
			// if response === 'done' you may want to go to done page here
